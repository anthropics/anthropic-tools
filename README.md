...
> This SDK is Currently Pre-Alpha. We promise no ongoing support.
...
```
NOTE: While we have attempted to validate tool_arguments before returning them to you, you may still want to do some additional checks of tool_arguments before executing the function to check for this like malicious or invalid parameters.
...
tool_inputs_message = {'role': 'tool_inputs', 'content': "Let's think this through. Maggie had 3 apples, she ate one so:", 'tool_inputs': [{'tool_name': 'perform_subtraction', 'tool_arguments': {'a': 3, 'b': 1}}]}
...
```python
human_message = {'role': 'human', 'content': 'If Maggie has 3 apples and eats 1, how many apples does maggie have left?'}
tool_inputs_message = {'role': 'tool_inputs', 'content': "Let's think this through. Maggie had 3 apples, she ate one so:", 'tool_inputs': [{'tool_name': 'perform_subtraction', 'tool_arguments': {'a': 3}}]}
tool_outputs_message = {
    'role': 'tool_outputs',
    'tool_outputs': None,
    'tool_error': 'Missing required parameter "b" in tool perform_addition.'
...
That's it for the new messages format. To help wrap your head around this concept, in the "Putting it Together" section below, we will build a python function to handle these sorts of requests.
...
Notice how this stops at the next message (in this case a `tool_inputs` message), and requires you to provide the `tool_outputs` message and pass in the new set of messages to keep going. Your next code would probably looke something like this:
```python
claude_res = {
    "role": "tool_inputs",
    "content": "Ok. Let's think through this in steps.\nSally has 17 apples.\nSally gives 9 apples to jim.\nso:\n",
...
def handle_manual_claude_res(messages, claude_res, tool_user):
    ...
    """
    messages.append(claude_res)
    # If the message is not trying to use a tool, we should not automatically respnd to Claude, and instead should ask the user for input
    if claude_res['role'] == "assistant":
        ...
        for tool_input in claude_res['tool_inputs']:
            ...
            tool_outputs.append({"tool_name": tool_input['tool_name'], "tool_result": tool_results})
