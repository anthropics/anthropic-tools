...
class TestToolUser(unittest.TestCase):
    ...
    def test_construct_injections(self):
        invoke_results_success = {"status": "SUCCESS", "invoke_results": [("perform_addition", 8), ("perform_addition", 4), ("perform_subtraction", -3)]}
    ...
    def test_function_calls_valid_format_and_invoke_extraction(self):
        ...
        self.assertEqual(self.tool_user._function_calls_valid_format_and_invoke_extraction("<function_calls><invoke><tool_name>perform_addition</tool_name><parameters><a>305</a><b>300</b></parameters></invoke></function_calls>"), {"status": True, "invokes": [{"tool_name": "perform_addition", "parameters_with_values": [("a", "305"), ("b", "300")]}]})

    def test_parse_function_calls(self):
        a = 300
        b = 305
        self.assertEqual(self.tool_user._parse_function_calls(f"some text that might go here<function_calls><invoke><tool_name>perform_addition</tool_name><parameters><a>{a}</a><b>{b}</b></parameters></invoke></function_calls>some more text that might go here..."), {"status": "SUCCESS", "invoke_results": [("perform_addition", addition_tool.use_tool(a, b))]})
